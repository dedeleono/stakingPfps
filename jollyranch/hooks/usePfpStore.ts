...
const usePfpStore = create<UsePfpStore>((set: any, get: any) => ({
    ...
    setupPfp: async (wallet: AnchorWallet) => {
        ...
        const program = new Program(shillCityIDL as anchor.Idl, shillCityCapital.toString(), provider);
    ...
    stakeNFT : async (nftPubKey: PublicKey) => {
        ...
        const [stake_spl, stakeBump] = await anchor.web3.PublicKey.findProgramAddress(
    ...
    resetStakedMints:() => {set({stakedMints: []})},
    resetStakedNFTs:() => {set({stakedNFTs: []})},
}));
